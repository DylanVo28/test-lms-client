...
interface NoDataContentProps {
  ...
  handleFindIdNextChildSection: (id: string) => any;
  handleFindIdPrevChildSection: (id: string) => any;
...
const NoDataContent = ({
  ...
  const isLastLesson = currentIndex === allItems?.length - 1;

  return (
    <div className="w-full relative min-h-[400px] md:min-h-[566px] flex justify-center pt-20 p-4 md:p-12 group">
      <LoadingContainer loading={loading} />

      {/* Previous Button - Hidden for first lesson */}
      {!isFirstLesson && dataItemPrev?.id && (
        <Button
      ...
      {/* Next Button - Hidden for last lesson */}
      {!isLastLesson && (
        <Button
