...
export function fromUnits(num: number | string, decimals: number | string = DEFAULT_DECIMALS) {
...
export const formatPriceValue = (numberStr: string, decimalPlaces = 5, fractionDigits = 3) => {
  ...
  if (numberStr.includes("e-")) {
    ...
    const valueDecimal = new Decimal(Number(value).toFixed(decimalPlaces).replace(".", ""));
    if (powDecimal.greaterThanOrEqualTo(fractionDigits)) {
      return `0.0_${powDecimal}_${valueDecimal.toString().slice(0, fractionDigits)}`;
    }
  }
